...
<jsp:useBean id="rg_login_DAO" class="Algorithm.LoginDAO"></jsp:useBean>
    ...
    ArrayList<Roogle_Cart_DTO> rgAry = rg_cart_DAO.mSelectCart(id);
    
    Roogle_Cart_DTO tempDTO = null;
    int resultPrice = 0;
    %>
				...
				<div id="왼쪽">
					...
					<div id="주문상품전체" class="메뉴">
						...
						<div id="내용2">
							<div class="내용배경색">
								<%
								for(int i = 0; i < rgAry.size(); i++){
									tempDTO = rgAry.get(i);
									resultPrice += tempDTO.getCart_price();
								%>
								<div class="주문상품">
								    <div class="장바구니품목블록">
								        <div class="상품이미지블록">
								            <img src="./../RoogleImg/<%=tempDTO.getCart_img()%>">
								        </div>
								        <div class="상품블록">
						...
						<h1>총 주문 금액 : <span id="주문금액"><%=resultPrice %></span>원</h1>
					</div>
								...
								<div class="상세정보칸">
									<div id="현금영수증_API">
										<div id="현금영수증변경전">
											...
											<p>
												<span class="전번"><%=loginDTO.getPhone().substring(0, 2) %></span> - 
												<span class="전번"><%=loginDTO.getPhone().substring(3, 6) %></span> - 
												<span class="전번"><%=loginDTO.getPhone().substring(7, 10) %></span>
											</p>
										</div>
										<div id="현금영수증변경">
					...
					<div id="내용5">
						<div class="내용배경색">
							<div class="상세정보칸">
								...
								<p style="text-align: right;"><span id="총결제금액"><%=resultPrice %></span>원</p>
							</div>
						</div>
						<button onclick="goPayPro('<%=id%>')">결제하기</button>
					</div>
				</div>
			</div>
